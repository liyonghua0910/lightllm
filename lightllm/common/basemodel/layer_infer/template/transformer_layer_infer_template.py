...
class TransformerLayerInferTpl(TransformerLayerInfer):
    ...
    def _h2_eviction(self, att_score, infer_state: InferStateInfo):
        ...
        max_h2_num = infer_state.req_manager.max_heavy_hitters_num
        local_num = infer_state.req_manager.local_tokens_num
        free_idxs = []
        ...
        if infer_state.is_prefill:
            for i in range(batch):  # 逐个序列分别处理
                ...
                if att_len <= cache_size:
                    ...
                    # 如果缓存预算不够, 那么用一部分预算存放 h2 tokens, 另一部分存放 local tokens
                    h2_pos = torch.argsort(score[:, :att_len-local_num], descending=True)[:,:max_h2_num]  # 在 local tokens 之外选择 top k
                    local_pos = torch.arange(att_len-local_num, att_len, device=h2_pos.device).repeat(head_num,1) # 生成 local tokens 的下标
                    selected_pos = torch.concat((h2_pos, local_pos), dim=1)
            ...
            for i in range(batch):  # 逐个序列分别处理
                ...
                if att_len <= cache_size:
                    ...
                    h2_indexs, local_indexs = torch.split(req_to_atten_indexs[req_idx, self.layer_num_], [max_h2_num, local_num], dim=1)
                    h2_scores, local_scores = torch.split(req_to_atten_scores[req_idx, self.layer_num_], [max_h2_num, local_num], dim=1)
                    if max_h2_num > 0:
                        evicted_score, evicted_pos = torch.min(h2_scores, dim=1)
                        for head in range(head_num):
                            if local_num > 0 and evicted_score[head] < local_scores[head, 0]:
                                # 如果是 h2+local 混合模式, 那么尝试用最久远的 local token 替换
                                h2_indexs[head, evicted_pos[head]] = local_indexs[head, 0]
                                h2_scores[head, evicted_pos[head]] = local_scores[head, 0]
                            elif local_num == 0 and evicted_score[head] < score[head, att_len-1]:
                                # 如果是 h2 only 模式, 那么尝试用最新生成的 token 替换
                                h2_indexs[head, evicted_pos[head]] = req_to_token_indexs[req_idx, seq_len-1]
                                h2_scores[head, evicted_pos[head]] = score[head, att_len-1]
                    # 踢掉最久远的 local token, 并加入最新的 token
                    local_indexs = req_to_token_indexs[req_idx, seq_len-local_num:seq_len].repeat(head_num, 1)
                    local_scores = torch.concat((local_scores[:, 1:], score[:, att_len-int(local_num>0):att_len]), dim=1)
                    assert local_indexs.shape[-1] == local_scores.shape[-1] == local_num
                    req_to_atten_indexs[req_idx, self.layer_num_] = torch.concat((h2_indexs, local_indexs), dim=1)
                    req_to_atten_scores[req_idx, self.layer_num_] = torch.concat((h2_scores, local_scores), dim=1)
