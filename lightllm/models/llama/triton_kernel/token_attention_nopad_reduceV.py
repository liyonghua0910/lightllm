...
@triton.jit
def _fwd_kernel_token_att2_h2o(
    Prob, V, Out, Req_to_tokens, B_req_idx, B_Start_Loc, B_Seqlen, 
    ...
    v_loc_off = cur_batch_req_idx * stride_req_to_tokens_b + cur_kv_head * stride_req_to_tokens_h + (cur_batch_start_index + offs_n) * stride_req_to_tokens_s
    p_offs = cur_head * stride_ph + (cur_batch_in_all_start_index + offs_n) * stride_pbs
    v_offs = cur_kv_head * stride_vh + offs_d[None, :] * stride_vd
...
@torch.no_grad()
def token_att_fwd2_h2o(prob, v, out, Req_to_tokens, B_req_idx, B_Start_Loc, B_Seqlen):
    if triton.__version__ >= "2.1.0":
        BLOCK = 128
    else:
        BLOCK = 64
    ...
    _fwd_kernel_token_att2_h2o[grid](
        prob, v, out, Req_to_tokens, B_req_idx, B_Start_Loc, B_Seqlen,
